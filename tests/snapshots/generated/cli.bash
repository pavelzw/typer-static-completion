...
_tsc_a0400f50fb4ab9f2() {
  ...
  if ((target < 0)); then
    ...
    0)
      candidates=(generate)
      if [[ $cur == -* && $ended == 0 ]]; then candidates=(--help); fi
      ;;
    1)
      candidates=()
      if [[ $cur == -* && $ended == 0 ]]; then candidates=(--prog-name --shell --output -o --help); fi
      ;;
    esac
    if [[ $cur != -* || $ended == 1 ]]; then
...
complete -F _tsc_a0400f50fb4ab9f2 -- typer-static-completion
